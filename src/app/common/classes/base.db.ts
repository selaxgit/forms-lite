...
export abstract class DBBase<T extends { id: number | null }> {
  protected abstract tableName: string;

  protected readonly dbVersion = 1;

  protected readonly dbName: string = 'db-forms-lite';

  protected readonly dbTables: string[] = ['forms', 'source-data', 'dto'];

  public removeByFilter(filter: (item: T) => boolean): Observable<void> {
    return this.getListByFilter(filter).pipe(
      switchMap((values: T[]) => {
        const ids = values.filter((item: T) => item.id !== null).map((item: T) => item.id) as number[];
        return this.batchRemove(ids).pipe(switchMap(() => of(undefined)));
      }),
    );
  }
